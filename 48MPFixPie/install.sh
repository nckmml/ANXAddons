...
ADDONID=48MP
TARGETANDROIDVERSION=9

anxconflict() {
  # Check if $1 is installed
  if [ -d "/data/adb/modules/$1" ] || [ -d "/sbin/.magisk/modules/$1" ]; then
    ui_print "   ! conflicting $1 is already installed!"; abort "   ! Remove $1 first !"
  fi
}

anx_check() {
  if [ $ADDONID = "48MP" ]; then
    anxconflict ANXCustLibs
  elif [ $ADDONID = "CUSTLIBS" ]; then
    anxconflict ANX48MPFix
  fi

  if [ $(getprop ro.build.version.release) != $TARGETANDROIDVERSION ]; then
    ui_print "   ! This addon is only for Android $TARGETANDROIDVERSION!"; abort "   ! Download correct addon !"
  fi
}
...
print_modname() {
  ...
  ui_print " "
  ui_print "************************************"
  ui_print "       48MP Fix for ANXCamera       "
  ui_print "        For Xiaomi's Devices        "
  ui_print "         Created by AEonAX          "
  ui_print "            with help of            "
  ui_print "         Dyneteve and Noid          "
  ui_print "************************************"
  ui_print " "
